...
class MapHelper{
  ...
  static Future<String?> getAddressFromLatLng(LatLng? po) async {
    try {
      ...
      List<Placemark> placemarks = await placemarkFromCoordinates(po.latitude, po.longitude);
  ...
  static Future<Position?> getCurrentLocation() async {
    ...
    if (permission == LocationPermission.deniedForever) {
      ToastHelper.showError(message: 'Location permissions are permanently denied, we cannot request permissions.');
      return null;
    }
    return await Geolocator.getCurrentPosition(desiredAccuracy: LocationAccuracy.high);
  }
  static onLocationChange({required Function(Position?) onChange})async{
